...
func GoCommand(_ *cobra.Command, _ []string) error {
	...
	if len(home) > 0 { //其次优先级是本地指定目录tpl文件目录，否则按照本地默认目录
...
func DoGenProject(apiFile, dir, style string) error {
	api, err := parser.Parse(apiFile) //通过ANTLR4解析.api文件, 按照g4模板是满足当前需求的，需转码内容要在Tag中手动解析
	if err != nil {
		return err
	}
	...
	cfg, err := config.NewConfig(style) //校验格式，是要驼峰还是swagger等形式
	if err != nil {
		return err
	}

	logx.Must(pathx.MkdirIfNotExist(dir))        //创建目录
	rootPkg, err := golang.GetParentPackage(dir) //包计算
	if err != nil {
		return err
	}
