...
func genFile(c fileGenConfig) error {
	...
	if len(c.category) == 0 || len(c.templateFile) == 0 {
		...
		text, err = pathx.LoadTemplate(c.category, c.templateFile, c.builtinTemplate) //读取tpl模板内容
		if err != nil {
			return err
		}
	}

	t := template.Must(template.New(c.templateName).Parse(text)) //解析tpl模板，得到待替换模板
	buffer := new(bytes.Buffer)
	err = t.Execute(buffer, c.data) //将tpl模板内容渲染数据(data)到buffer
	if err != nil {
		return err
	}

	code := golang.FormatCode(buffer.String()) //格式化生成的代码
	_, err = fp.WriteString(code)              //将格式化后代码写入文件
	return err
}
