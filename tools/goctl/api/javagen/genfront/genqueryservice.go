...
//go:embed fronttpl/QueryService.tpl
var queryServiceTemplate string

func GenQueryService(dir, svName, subSvName string, api *spec.ApiSpec, ch chan bool) error {

	defer func() { ch <- true }()
	var imports string

	javaFile := "Query" + javagenutil.HeadUpCase(subSvName) + "Service.java"
	fp, created, err := apiutil.MaybeCreateFile(dir+"/"+subSvName+"/service/query", "", javaFile)
	if err != nil {
		return err
	} //创建出现错误
	if !created {
	...
	tpl := template.Must(template.New(dir + "queryServiceTemplate").Parse(queryServiceTemplate))
	err = tpl.Execute(&tmplBytes, map[string]any{ //渲染模板了
		"svName":      strings.ToLower(svName),
		"subSvName":   strings.ToLower(subSvName),
		"imports":     imports,
		"serviceName": front.AssembleQueryService(svName, subSvName, nil),
