...
//go:embed fronttpl/RemoteService.tpl
...
func GenRemoteService(dir, svName, subSvName string, api *spec.ApiSpec, ch chan bool) error {
	...
	fp, created, err := apiutil.MaybeCreateFile(dir+"/"+subSvName+"/remote/servertransfer", "", javaFile)
	if err != nil {
		return err
	} //创建出现错误
	if !created {
	...
	err = tpl.Execute(&tmplBytes, map[string]any{ //渲染模板了
		...
		"interfaceName":    front.AssembleRemoteService(svName, subSvName, nil),
		"imports":          imports,
