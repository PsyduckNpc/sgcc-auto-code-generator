...
//go:embed fronttpl/Controller.tpl
var controllerTemplate string

func GenController(dir, svName, subSvName string, api *spec.ApiSpec, ch chan bool) error {

	defer func() { ch <- true }()

	javaFile := front.AssembleController(svName, subSvName, nil) + ".java"
	fp, created, err := apiutil.MaybeCreateFile(dir+"/"+subSvName+"/controller", "", javaFile)
	if err != nil {
		return err
	} //创建出现错误
	if !created {
