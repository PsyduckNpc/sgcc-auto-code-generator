...
//go:embed fronttpl/QueryServiceImpl.tpl
var queryServiceTemplateImpl string

func GenQueryServiceImpl(dir, svName, subSvName string, api *spec.ApiSpec, ch chan bool) error {

	defer func() { ch <- true }()

	javaFile := front.AssembleQueryServiceImpl(svName, subSvName, nil) + ".java"
	fp, created, err := apiutil.MaybeCreateFile(dir+"/"+subSvName+"/service/query/impl", "", javaFile)
	if err != nil {
		return err
	} //创建出现错误
	if !created {
	...
	injects = append(injects, javagenutil.Inject{
		InjectServiceName:   front.AssembleQueryRemoteService(svName, subSvName, &importMap),
		VariableServiceName: javagenutil.HeadLowCase(front.AssembleQueryRemoteService(svName, subSvName, nil)),
	})
	...
	for _, v := range api.Service.Routes() {
		...
		logics = append(logics, javagenutil.Logic{
			...
			VariableServiceName: javagenutil.HeadLowCase(front.AssembleQueryRemoteService(svName, subSvName, &importMap)),
	...
	err = tpl.Execute(&tmplBytes, map[string]any{ //渲染模板了
		...
		"imports":         imports,
		"serviceImplName": front.AssembleQueryServiceImpl(svName, subSvName, nil),
