...
func GenRemoteServiceImpl(dir, svName, subSvName string, api *spec.ApiSpec, ch chan bool) error {
	...
	fp, created, err := apiutil.MaybeCreateFile(dir+"/remote/datatransfer/impl", "", javaFile)
	if err != nil {
		return err
	} //创建出现错误
	if !created {
	...
	err = tpl.Execute(&tmplBytes, map[string]any{ //渲染模板了
		...
		"imports":       imports,
		"className":     front.AssembleRemoteServiceImpl(svName, subSvName, nil),
		"implementName": interfaceImplName,
		"inters":        inters,
	})
	if err != nil {
		return err
