...
func genPacket(dir, packetName string, api *spec.ApiSpec) error {
	for _, route := range api.Service.Routes() { //便利routes， api文件中在同一路由组中的接口将在一个controller中出生成
...
func createWith(dir string, api *spec.ApiSpec, route spec.Route, packetName string) error {
	...
	javaFile := packet + ".java"                                   //文件全路径
	fp, created, err := apiutil.MaybeCreateFile(dir, "", javaFile) //创建java文件
	if err != nil {
		return err
	}
	if !created {
	...
	params := strings.TrimSpace(paramsForRoute(route)) //解析路由参数
	if len(params) > 0 && hasRequestBody {             //如果有路由参数或者请求体参数(Body参数或者Post中的表单参数)
		params += ", "
	}
	paramsDeclaration := declarationForRoute(route) //生成路由参数
	paramsSetter := paramsSet(route)                //路由参数setter
	imports := getImports(api, packetName)          //导入import

	if len(route.ResponseTypeName()) == 0 { //响应内容为结构体的话，添加响应参数的import
		imports += fmt.Sprintf("\v%s", "import com.xhb.core.response.EmptyResponse;")
	}

	t := template.Must(template.New("packetTemplate").Parse(packetTemplate)) //解析模板了
	var tmplBytes bytes.Buffer
	err = t.Execute(&tmplBytes, map[string]any{ //渲染模板了
		"packetName":        packet,
...
func paramsForRoute(route spec.Route) string {
	...
	for _, cop := range cops {
		...
		if strings.HasPrefix(cop, ":") { //路由传参时
