...
type (
	// Type describes structure ast for api syntax
	//Type struct {
	//	AtType  *AtType
	//	TypeApi *TypeExpr
	//}
	...
	// AtType describes structure ast for api syntax
	AtType struct {
		AtTypeToken Expr
		Lp          Expr
		Rp          Expr
...
// VisitAtType implements from api.BaseApiParserVisitor
func (v *ApiVisitor) VisitAtType(ctx *api.AtTypeContext) interface{} {
	var atType AtType
	atType.AtTypeToken = v.newExprWithTerminalNode(ctx.ATTYPE())

	if ctx.STRING() != nil {
		atType.LineDoc = v.newExprWithTerminalNode(ctx.STRING())
	} else {
		for _, each := range ctx.AllKvLit() {
			atType.Kv = append(atType.Kv, each.Accept(v).(*KvExpr))
		}
	}
	atType.Lp = v.newExprWithToken(ctx.GetLp())
	atType.Rp = v.newExprWithToken(ctx.GetRp())

	if ctx.GetLp() != nil {
		if ctx.GetRp() == nil {
			v.panic(atType.Lp, "mismatched ')'")
		}
	}

	if ctx.GetRp() != nil {
		if ctx.GetLp() == nil {
			v.panic(atType.Rp, "mismatched '('")
		}
	}
...
func (v *ApiVisitor) VisitTypeLit(ctx *api.TypeLitContext) any {
	...
	if ok {
		if ctx.AtType() != nil {
			alias.AtType = ctx.AtType().Accept(v).(*AtType)
		}
		return alias
	}
	...
	if ok {
		if ctx.AtType() != nil {
			st.AtType = ctx.AtType().Accept(v).(*AtType)
		}
		st.DocExpr = doc
		return st
	}
