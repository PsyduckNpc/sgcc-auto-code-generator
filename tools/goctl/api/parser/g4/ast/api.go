...
type Api struct {
	...
	Info       *InfoExpr
	SvComm     []SvCommExpr
	svCommM    map[string]PlaceHolder
	Type       []TypeExpr
...
func (v *ApiVisitor) VisitApi(ctx *api.ApiContext) any {
	...
	final.svCommM = map[string]PlaceHolder{}
...
func (v *ApiVisitor) acceptSvComm(root, final *Api) {
	for _, tp := range root.SvComm {
		if _, ok := final.svCommM[tp.SvCommToken.Text()]; ok {
			v.panic(tp.SvCommToken, fmt.Sprintf("duplicate svcomm '%s'", tp.SvCommToken.Text()))
		}

		final.svCommM[tp.SvCommToken.Text()] = Holder
		final.SvComm = append(final.SvComm, tp)
	}
}
...
func (v *ApiVisitor) VisitSpec(ctx *api.SpecContext) any {
	...
	if ctx.SvcommLit() != nil {
		root.SvComm = ctx.SvcommLit().Accept(v).([]SvCommExpr)
	}
