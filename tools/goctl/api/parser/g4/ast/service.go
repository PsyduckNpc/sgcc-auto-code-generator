...
func (v *ApiVisitor) VisitServiceRoute(ctx *api.ServiceRouteContext) any {
	...
	if ctx.AtDesc() != nil {
		serviceRoute.AtDesc = ctx.AtDesc().Accept(v).(*AtDesc)
	}
...
// VisitAtDesc implements from api.BaseApiParserVisitor
func (v *ApiVisitor) VisitAtDesc(ctx *api.AtDescContext) any {
	var atDesc AtDesc
	astDescExpr := v.newExprWithTerminalNode(ctx.ATDESC())
	atDesc.AtDescToken = astDescExpr
	atDesc.Name = v.newExprWithTerminalNode(ctx.ID())
	atDesc.DocExpr = v.getDoc(ctx)
	atDesc.CommentExpr = v.getComment(ctx)
	return &atDesc
}
...
func (a *AtDesc) Doc() []Expr {
	return a.DocExpr
}

func (a *AtDesc) Comment() Expr {
	return a.Comment()
}

func (a *AtDesc) Format() error {
	return nil
}

func (a *AtDesc) Equal(v any) bool {
	if v == nil {
		return false
	}
	...
	if !a.AtDescToken.Equal(atDesc.AtDescToken) {
		return false
	}

	if !a.Name.Equal(atDesc.Name) {
		return false
	}

	return EqualDoc(a, atDesc)
}
